...
namespace LZ::Renderer {
	...
	CubeMapTexture::CubeMapTexture(const Wrapper::Device::Ptr& device, const Wrapper::CommandPool::Ptr& commandPool, const std::string& imageFilePath) {
		...
		LZ::Tools::Bitmap hdrBitmap(width, height, 4, LZ::Tools::BitmapFormat::Float, hdrData);
		stbi_image_free(hdrData);

		// 2 Hdr to cube map
		LZ::Tools::Bitmap cubemapBitmap = LZ::Tools::Tools::convertEquirectangularToCubemapFaces(hdrBitmap);
		...
		const int bytesPerPixel = cubemapBitmap.mChannels * LZ::Tools::Bitmap::getBytesPerChannel(cubemapBitmap.mFormat);
