...
public:
    ...
    static Ptr create(const Wrapper::Device::Ptr& device, const Wrapper::CommandPool::Ptr& commandPool, MeshData& meshData, const Scene& scene) {

        return std::make_shared<SceneMeshRenderer>(device, commandPool, meshData, scene);
    }

	SceneMeshRenderer(const Wrapper::Device::Ptr& device, const Wrapper::CommandPool::Ptr& commandPool, MeshData& meshData, const Scene& scene);
	~SceneMeshRenderer();

    void loadTextures(const Wrapper::Device::Ptr& device, const Wrapper::CommandPool::Ptr& commandPool, MeshData& meshData);
    void draw(const Wrapper::CommandBuffer::Ptr& cmd);
...
private:
    ...
    Wrapper::Buffer::Ptr mIndirectBuffer{ nullptr };
